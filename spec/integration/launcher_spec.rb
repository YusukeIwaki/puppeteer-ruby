...
RSpec.describe Puppeteer::Launcher do
  ...
  describe 'Puppeteer#launch', puppeteer: :browser do
    ...
    it 'should report a permission error over pipe transport when the user_data_dir is not writable' do
      # Windows ignores the read-only bit for the directory owner, and root
      # bypasses the write check entirely, so neither can observe the failure.
      skip('Not observable on Windows or as root') if Puppeteer.env.windows? || Process.uid == 0
  ...
  describe 'Puppeteer.connect', puppeteer: :browser do
    ...
    it 'should be able to reconnect to a disconnected browser', sinatra: true do
      ...
      begin
        ...
        begin
          restored_page = remote_browser.pages.find do |p|
            p.url == "#{server_prefix}/frames/nested-frames.html"
          end
