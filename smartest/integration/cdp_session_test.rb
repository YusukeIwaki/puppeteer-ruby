...
describe Puppeteer::CDPSession do
  ...
  it 'should not report created targets for custom CDP sessions', puppeteer: :browser do
    called = false
    browser_context = browser.browser_contexts.first
    listener_id = browser_context.on('targetcreated') do |target|
    ...
    browser_context&.off(listener_id)
  end
